...
def get_img_texture_features(
    path2img,
    crop=(50, 300, 2700, 4200),
    resize_to=[1200, 800],
    box_size=[200, 300],
    npatches=20,
    patch_size=30,
    ...
    img_patches = [
        generate_random_img_patches(I, patch_size=patch_size, num_locs=npatches)
        for I in img_list
    ]
...
def get_all_texture_mapping_features(
    path_to_images,
    crop=(50, 300, 2700, 4200),
    resize_to=[800, 1200],
    box_size=[600, 800],
    ...
    for d in dirs:
        ...
        for f in fls:
            ...
            if b < 300e3:
                # blank dark image
                continue
            x = get_img_texture_features(
                path2img, crop, resize_to, box_size, npatches, patch_size, verbose=False
            )
            cnt += 1
            X.append(x)
            if cnt % verbose_every == 0:
                print(
                    " done with %d images , feature space size %d " % (cnt, x.shape[-1])
                )
    return X
...
def generate_random_img_locations(img, patch_size=5, num_locs=5):
    ...
    H, W = img.size
    idx_rows = np.random.choice(
        np.arange(patch_size, H - patch_size), size=num_locs, replace=False
    )
    idx_cols = np.random.choice(
        np.arange(patch_size, W - patch_size), size=num_locs, replace=False
    )
    ...
    for loc in locations:
        patches.append(
            np.array(
                img.crop([loc[0], loc[1], loc[0] + patch_size, loc[1] + patch_size])
            )
        )
    return patches
...
def generate_random_img_patches(img, patch_size=5, num_locs=5):
    ...
    W = im.shape[1]
    idx_rows = np.random.choice(
        np.arange(patch_size, H - patch_size), size=num_locs, replace=True
    )
    idx_cols = np.random.choice(
        np.arange(patch_size, W - patch_size), size=num_locs, replace=True
    )
    locations = [(r, c) for (r, c) in zip(idx_rows, idx_cols)]
    patches = []
    for loc in locations:
