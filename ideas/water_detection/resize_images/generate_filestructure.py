...
remove_corrupted_images = (
    True  # each image will checked if it loads correctly. Takes more time
)
    ...
    for i in range(len(img_file_list)):
        ...
        try:
            ...
            try:
                ...
                warnings.warn(
                    "The following is not a valid image filename and will be ignored: %s"
                    % img_file
                )
                continue
        ...
        if last_timestamp is not None:
            if timestamp - last_timestamp < timedelta(
                minutes=3.7
            ):  # add some error margin 3.7 minutes instead of 4
        ...
        dst = (
            ...
            + ".JPG"
        )  # create filename with new convention
        ...
        if img_file.suffix in [".JPG", ".jpg"]:
            ...
            cmd = shlex.split(
                "./NConvert/nconvert -quiet -out jpeg -embedded_jpeg -resize %d %d -overwrite -o %s %s"
                % (resize_w, resize_h, dst, src)
            )
            ...
            warnings.warn(
                "The file has an unknown extension an will be ignored: %s" % img_file
            )
