...
#################################################################################
dumb_prediction = np.asarray( [[None] * len(rock_temperature[1, :]) for i in range(len(rock_temperature[:, 1])) ] )  
...
diff_prediction = np.asarray( [[None] * len(rock_temperature[1, :]) for i in range(len(rock_temperature[:, 1])) ] )  
for depth in range(len(rock_temperature[1, :])):
    diff_prediction[1, depth] = None
    for element in range(1, len(dumb_prediction[:, depth])):
        if diff_prediction[element - 1, depth] == None or np.isnan(diff_prediction[element - 1, depth]) or element <= 25:
            diff_prediction[element, depth] = rock_temperature.values[element - 1, depth]
        else:
            diff_prediction[element, depth] = rock_temperature.values[element - 1, depth] + (rock_temperature.values[element - 24, depth] - rock_temperature.values[element - 25, depth]) 
...
def absolute_error(prediction, original):
    ...
    for depth in range(len( _ae[1, :])):
        for element in range(len( _ae[:, depth])):
            if not (prediction[element, depth] == None or np.isnan(prediction[element, depth])):
                _ae[element, depth] = np.abs( original[element, depth] - prediction[element, depth] )
    return _ae

def rmse(_ae):
    _rmse = np.asarray( [None] * len(_ae[1, :]) ) 
    for depth in range(len( _ae[1, :])):
        filtered_ae = np.asarray([i for i in _ae[:, depth] if not (i == None or np.isnan(i)) ])
...
for i in range(1): #rock_temperature.shape[1]):
    fig.add_trace(
        go.Scatter(
            ...
            name="actual_" + rock_temperature['name'].values[i] + ' [' + rock_temperature['unit'].values[i] + ']'
        )
    )
...
for i in range(1):
    fig.add_trace(
        go.Scatter(
            ...
            name="dumb_pred_" + rock_temperature['name'].values[i] + ' [' + rock_temperature['unit'].values[i] + ']'
        )
    )
...
for i in range(1):
    fig.add_trace(
        go.Scatter(
            ...
            name="diff_pred_" + rock_temperature['name'].values[i] + ' [' + rock_temperature['unit'].values[i] + ']'
        )
    )    
...
for i in range(1):
    fig.add_trace(
        go.Scatter(
            ...
            name="dumb_a_error_" + rock_temperature['name'].values[i] + ' [' + rock_temperature['unit'].values[i] + ']'
        )
    )     
...
for i in range(1):
    fig.add_trace(
        go.Scatter(
            ...
            name="diff_a_error_" + rock_temperature['name'].values[i] + ' [' + rock_temperature['unit'].values[i] + ']'
        )
    )       
